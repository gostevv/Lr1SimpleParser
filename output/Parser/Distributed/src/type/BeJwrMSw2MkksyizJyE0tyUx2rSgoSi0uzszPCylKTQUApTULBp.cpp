...
#include "BeJwrMSw2MkksyizJyE0tyUx2rSgoSi0uzszPCylKTQUApTULBp.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring arithmeticExpressionTree>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("arithmeticExpressionTree");
        TypeOffset t(MY_OFFSETOF(arithmeticExpressionTree_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "arithmeticExpressionTree") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, arithmeticExpressionTree_);
   else
     istr >> arithmeticExpressionTree_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, arithmeticExpressionTree_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{'
         << "arithmeticExpressionTree=" << get_arithmeticExpressionTree()  
         << '}';
}
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "arithmeticExpressionTree=", get_arithmeticExpressionTree()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_arithmeticExpressionTree());
