...
#include "BeJwrMczxMs9NLS5OTE8FABvbAR4.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<list<uint8> message>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    
    {
        ...
        TypeOffset t(MY_OFFSETOF(message_, tm), 
                     Meta::Type::typeOf<SPL::list<SPL::uint8 > >(), 
                     &typeid(SPL::list<SPL::uint8 >));
        addMapping(*tm, t, s, 0);
    }
...
SELF& SELF::clear()
{
    if (get_message().size() < 1024) {
        get_message().clear();
    }
    else {
        SPL::list<SPL::uint8 >().swap(get_message());
    }
