...
#include "BeJwrMSw2Mk3LzEkNzyzJcK0oKEotLs7MzwvJD0gsKk4FAK5VAuq.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring fileWithExpressionToParse>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("fileWithExpressionToParse");
        TypeOffset t(MY_OFFSETOF(fileWithExpressionToParse_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "fileWithExpressionToParse") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, fileWithExpressionToParse_);
   else
     istr >> fileWithExpressionToParse_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, fileWithExpressionToParse_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{'
         << "fileWithExpressionToParse=" << get_fileWithExpressionToParse()  
         << '}';
}
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "fileWithExpressionToParse=", get_fileWithExpressionToParse()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_fileWithExpressionToParse());
